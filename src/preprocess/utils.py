...
def get_dataset(path: str) -> Tuple[pd.Series, pd.Series]:
    """
...
class DataBuilder:
    ...
    def clean_text(self, content: str) -> str:
        ...
        txt = [w for w in txt if not self.nlp.vocab[str(w)].is_stop and not w.isnumeric()]
        txt = [w for w in txt if w not in self.stop]
        txt = [w for w in txt if not w.startswith(tuple(self.remove))]
    ...
    def build_dataset(self) -> None:
        ...
        final_df.TEXT = final_df.TEXT.apply(self.clean_text)
